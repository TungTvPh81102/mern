...
const ListBrand = () => {
  ...
  useEffect(() => {
    setData(state.brands);
  }, [state.brands]);

  const columns: ColumnDef<IBrands>[] = [
    {
      accessorKey: "id",
      header: "STT",
    ...
    },
    {
      ...
      header: "Logo",
      cell: ({ row }) => (
        <img width={100} height={50} src={row.getValue("thumbnail")} alt="" />
      ),
    },
    {
      accessorKey: "status",
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-4 items-center">
          <Link to={`/manager/brands/edit/${row.original._id ?? ""}`}>
            <IconEdit />
          </Link>
